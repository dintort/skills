...
Determine `BASE_BRANCH`: if the user specified a base branch, e.g. using words like "against", "vs", "compare", "base", etc., then use the specified branch name; otherwise use `HEAD`.
Determine `DIFF_FILE` as `ROAST-{yyyyMMdd-HHmmss}-{id-short-feature-name}.diff` — get the actual current date and time by running a shell command - do not infer or guess.
...
Access the relevant project files for surrounding context and details.
...
Create an MD file at project root with the review results.
Name the file as `ROAST-{yyyyMMdd-HHmmss}-{id-short-feature-name}.md` using the same date/time as for the diff file.
Check if `.git/info/exclude` already contains `ROAST-*` and if not, append it using the appropriate command for the platform:
...
Use `git rm --cached --ignore-unmatch` for both the diff file and the report file to untrack them in git in case IDE automatically staged them.
