...
1. Sync remote branches. Execute:
```bash
git fetch
```
...
3. Determine `DIFF_FILE` as `ROAST-{yyyyMMdd-HHmmss}-{id-short-feature-name}.diff` — get the actual current date and time by running a shell command - do not infer or guess.
- Replace `BASE_BRANCH` and `DIFF_FILE` in the command below and execute:
```bash
git diff origin/BASE_BRANCH...HEAD --stat && git diff origin/BASE_BRANCH...HEAD > DIFF_FILE
```
4. Make sure to read the diff file in full - not truncated or grepped. If the file is too large to read in a single call, read it in chunks using ranged reads.
5. Analyze and review the diff for potential bugs, logical errors, edge cases, correctness, security, vulnerabilities, performance issues, concurrency issues, race conditions, injection risks, auth issues, data exposure, N+1 queries, unnecessary loops, memory leaks, complexity, duplication, backward compatibility, style, formatting, test coverage, and any other issues.
6. Validate if the pull request fully addresses the issue in question.
7. Suggest improvements for readability, maintainability, and naming conventions. Provide specific refactoring suggestions.
8. Be harsh, merciless, and do criticize. I'd rather fix issues now than in production.
9. Access the relevant project files for surrounding context and details.
10. Number each issue sequentially.
11. For each issue mention:
...
12. Create an MD file at project root with the review results.
- Name the file as `ROAST-{yyyyMMdd-HHmmss}-{id-short-feature-name}.md` using the same date/time as for the diff file.
13. Check if `.git/info/exclude` already contains `ROAST-*` and if not, append it using the appropriate command for the platform:
...
14. Use `git rm --cached --ignore-unmatch` for both the diff file and the report file to untrack them in git in case IDE automatically staged them.
